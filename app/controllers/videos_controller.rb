class VideosController < ApplicationController
  ...
  def rated
    ...
    @rated_videos = @rated_videos.paginate(page: params[:rated_videos], per_page: 10)
  end
  ...
  def search
    if params[:text_search]
      @searched_videos = Video.where(approved: true).includes(:category, :user).text_search(params[:text_search]).first(12)
    end
  end
end
