task calculate_rating: :environment do
  ...
  votes_sql = "select id, ntile(180) over (order by votes_count) from videos where approved = true"
 ...
 # calculate_plays_quartile
  plays_sql = "select id, ntile(180) over (order by plays_count) from videos where approved = true"
  ...
  shares_sql = "select id, ntile(180) over (order by shares_count) from videos where approved = true"
  ...
  # calculate_rating
  videos.each{|v| v.update_attribute(:rating, ((v.shares_quartile * 0.0222 + v.plays_quartile * 0.0222 + v.votes_quartile * 0.0222 + 0.8 * v.ave_vote) / 4).round(5) )}
end
