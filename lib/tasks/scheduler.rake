task calculate_rating: :environment do
  ...
  votes_sql = "select id, ntile(4) over (order by votes_count) from videos where approved = true"
 ...
 # calculate_plays_quartile
  plays_sql = "select id, ntile(4) over (order by plays_count) from videos where approved = true"
  ...
  shares_sql = "select id, ntile(4) over (order by shares_count) from videos where approved = true"
